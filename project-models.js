...
async function mount(host){
 ...
 const size=()=>{const w=host.clientWidth,h=host.clientHeight-(type==='asl'&&w<=520?(w<=320?165:130):0);renderer.setSize(w,h);model.resize?.(w,h);camera.aspect=w/h;camera.position.z=type==='asl'&&w<=520?10.3:camera.aspect<1?10.5:8.4;camera.updateProjectionMatrix();state.dirty=true};new ResizeObserver(size).observe(host);size();
