...
function documentTexture(title,accent='#a997eb',kind='document'){
...
function base(parent,color){const m=mesh(new THREE.CylinderGeometry(2.15,2.22,.13,64),material(0x253741,{metalness:.4}),parent,0,-1.90,0);const ring=mesh(new THREE.TorusGeometry(2.17,.014,8,90),material(color,{emissive:color,emissiveIntensity:.3}),parent,0,-1.82,0);ring.rotation.x=Math.PI/2;return m}

function handModel(root,color){
 const cyan=material(color,{emissive:color,emissiveIntensity:.12}),muted=material(0x35566b),joint=material(0xc5edff,{emissive:color,emissiveIntensity:.18});
 ...
 const bones=pairs.map(([a,b])=>rod(hand,points[a],points[b],.044,cyan));
 const palmGeo=new THREE.BufferGeometry();const palmIndices=[0,1,5,9,13,17];const vertices=[];for(let i=1;i<palmIndices.length-1;i++)for(const j of [palmIndices[0],palmIndices[i],palmIndices[i+1]])vertices.push(...points[j].toArray());palmGeo.setAttribute('position',new THREE.Float32BufferAttribute(vertices,3));palmGeo.computeVertexNormals();mesh(palmGeo,material(0x4784a6,{transparent:true,opacity:.3,side:THREE.DoubleSide,depthWrite:false}),hand);
 const measurements=[[4,8],[8,12],[12,16],[16,20],[0,12]].map(([a,b])=>rod(hand,points[a],points[b],.008,material(0xf2be70,{transparent:true,opacity:.8})));
 const labels=[['21','LANDMARKS'],['75','FEATURES'],['A–Z','26 CLASSES']].map(([big,small])=>texture(256,320,c=>{c.fillStyle='#132a3b';c.fillRect(0,0,256,320);text(c,big,128,147,62,'#94ddff','center');text(c,small,128,202,19,'#9bb5c6','center');c.strokeStyle='#54768a';c.strokeRect(14,14,228,292)}));
 ...
 return {setStep(step){panel.face.material.map=labels[step];measurements.forEach(m=>m.visible=step===1);bones.forEach(m=>m.material=step===2?muted:cyan);joints.forEach(j=>j.material=step===1?cyan:joint)},update(t,step,still){scanner.position.y=still?.3:-1.35+(Math.sin(t*.8)+1)*1.55;scanner.visible=step===0;hand.rotation.y=still?-.07:Math.sin(t*.38)*.12;}};
}
function hrModel(root,color){
 base(root,color);const people=new THREE.Group();people.position.set(-1.30,0,-.25);root.add(people);const mat=material(color),dark=material(0x657598);
 const person=(x,y,z,s)=>{const g=new THREE.Group();g.position.set(x,y,z);g.scale.setScalar(s);people.add(g);ball(g,.28,mat,0,.75);mesh(new THREE.CapsuleGeometry(.30,.42,8,16),dark,g,0,.02,0);const disc=mesh(new THREE.CylinderGeometry(.62,.65,.1,32),material(0x37405d),g,0,-.55,0);return g};
 person(-.60,-.2,.35,1);person(.48,.35,-.55,.82);person(.75,-.6,.55,.70);
 const cert=card(root,1.90,2.6,documentTexture('CERTIFICATE','#a997eb','certificate'),1.35,.05,.20);cert.group.rotation.set(-.06,-.18,.035);
 const request=card(root,.88,1.2,documentTexture('REQUEST','#a997eb','profile'),-.25,-.75,1.10);request.group.rotation.z=-.15;
 ...
 return {setStep(step){seal.visible=step===2;request.group.visible=step!==2;people.traverse(o=>{if(o.isMesh)o.material.emissive?.setHex(step===0?0x19122d:0x000000)})},update(t,step,still){orbit(still?2:t);cert.group.position.z=THREE.MathUtils.lerp(cert.group.position.z,step===2?.75:.2,still?1:.065);request.group.position.y=-.65+(still?0:Math.sin(t*.7)*.08);}};
}
function employeeModel(root,color){
 base(root,color);const cabinet=new THREE.Group();cabinet.position.set(-.70,-.20,0);root.add(cabinet);const frame=material(0x4c6961),front=material(0x6f9282),lime=material(color,{emissive:color,emissiveIntensity:.08});
 box(cabinet,2.25,2.72,.85,frame,0,0,-.22);box(cabinet,2.3,.13,.94,material(0x9caf99),0,1.42,-.22);const drawers=[];
 for(let i=0;i<3;i++){const g=new THREE.Group();g.position.set(0,.9-i*.88,.27);cabinet.add(g);box(g,2.04,.73,.75,front,0,0,-.35);box(g,1.0,.075,.11,lime,0,.10,.07);box(g,.44,.13,.016,material(0xdce6d8),-.58,-.16,.043);for(let j=0;j<4;j++)box(g,1.8,.12,.045,material(j===1?color:0xcedacd),0,.3,-.15-j*.13);drawers.push(g)}
 const profile=card(root,1.35,1.85,documentTexture('PROFILE','#91bc75','profile'),1.58,.32,.72);profile.group.rotation.set(.02,-.22,-.05);
 const bars=new THREE.Group();bars.position.set(1.62,-1.55,.5);root.add(bars);[.5,.8,.65,1.0].forEach((h,i)=>box(bars,.18,h,.18,lime,(i-1.5)*.31,h/2,0));
...
function resumeModel(root,color){
 ...
 const backing=card(page,2.55,3.6,texture(512,720,c=>{c.fillStyle='#f5eee7';c.fillRect(0,0,512,720);c.fillStyle='#e2a37f';c.fillRect(0,0,512,12);text(c,'YOUR NAME',40,83,36);text(c,'PROFESSIONAL PROFILE',40,117,14,'#8d796b')}));
 const sectionNames=['EXPERIENCE','EDUCATION','SKILLS'];const layers=sectionNames.map((name,i)=>{const t=texture(440,150,c=>{c.fillStyle='#fffaf4';c.fillRect(0,0,440,150);text(c,name,22,35,18,'#8b624b');for(let j=0;j<3;j++){c.fillStyle='#d9cbc0';rounded(c,22,56+j*25,350-j*35,8,4)}});const piece=card(page,2.18,.73,t,0,.43-i*.92,.10);return piece.group});
 const languageTextures=['EN','العربية'].map((lang,i)=>texture(240,160,c=>{c.fillStyle='#50382b';c.fillRect(0,0,240,160);text(c,lang,120,98,i?41:58,'#ffd0ad','center')}));
 ...
 const pdf=card(root,1.07,1.47,texture(256,352,c=>{c.fillStyle='#ecd9c9';c.fillRect(0,0,256,352);text(c,'PDF',128,158,60,'#644333','center');text(c,'DOCUMENT',128,205,18,'#97755a','center');c.strokeStyle='#a97856';c.lineWidth=4;c.strokeRect(18,18,220,316)}),1.75,-.77,.65);pdf.group.rotation.z=.12;
...
function mount(host){
 ...
 const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(36,1,.1,50);camera.position.set(0,.65,8.4);camera.lookAt(0,0,0);scene.add(new THREE.HemisphereLight(0xe6f3ff,0x3a4752,2.4));const key=new THREE.DirectionalLight(0xffffff,3.2);key.position.set(-3,5,6);scene.add(key);const fill=new THREE.DirectionalLight(def.color,2);fill.position.set(4,1,3);scene.add(fill);
